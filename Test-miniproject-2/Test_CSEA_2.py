...
config_net={
    # Net params
    'features' : 64,                                # nb of channels in encoding layers
    'kernel_size' : 3,                                 # fixed kernel size for all convolutional layers
}
...
valid_input, valid_target = torch.load(path_data+'/'+'val_data.pkl')#validation set (noise-clean)
train_input, train_target = torch.load(path_data+'/'+'train_data.pkl') #test set (noise-noise)
...
t_train=time.time()
net.train(train_input, train_target, filename=filename)
