...
class _Encoder_Block(nn.Module):
    def __init__(self, in_channles, out_channels, conv_ksize, maxp_ksize):
        ...
        self.relu = nn.ReLU(out_channels) #nn.LeakyReLU(inplace=True)
        
    def forward(self, x):
        x = torch.relu(self.conv(x)) #convolution
...
class _Decoder_Block(nn.Module):
    def __init__(self, in0, in1, out1, conv_ksize):
        ...
        self.relu0 = nn.ReLU(in1) #nn.LeakyReLU(inplace=True)
        self.relu1 = nn.ReLU(out1)
        
    def forward(self, x, y):
        x = F.interpolate(x, scale_factor=2, mode='nearest') #upsample
        x = torch.cat((x,y),dim=1)    #concatenate
        x = torch.relu(self.conv0(x)) #first convolution 
        x = torch.relu(self.conv1(x)) #second convlution
        return x
    ...
    def train(self, train_input, train_target, filename=None,  val_input=None, val_target=None,) -> None:
        ...
        for epoch in range(self.num_epochs):
            ...
            if val_input is not None and val_target is not None:
                ...
                if filename is not None:
                    with open(filename, 'a') as file:
                        file.write("%d\t %.10g\t %.10g\t %.3f"%(epoch, acc_loss_per_sample, mse_per_sample, psnr)+'\n')
